...
const Dashboard = () => {
  ...
  useEffect(() => {
    console.log(
      "🌀 [Stripe useEffect] Déclenché avec :",
      profile?.stripe_customer_id
    );

    if (!profile?.stripe_customer_id) {
      console.warn(
        "⚠️ [Stripe useEffect] Aucun stripe_customer_id dans le profil"
      );
      return;
    }
    ...
    const fetchStripeData = async () => {
      console.log(
        "🚀 [fetchStripeData] Lancement avec stripeCustomerId :",
        profile.stripe_customer_id
      );
      ...
      try {
        ...
        console.log("📩 [fetchStripeData] Réponse brute reçue :", response);

        if (!response.ok) {
          console.error(
            "❌ [fetchStripeData] Erreur HTTP :",
            response.status,
            response.statusText
          );
          return;
        ...
        console.log(
          "✅ [fetchStripeData] Données Stripe décodées :",
          stripeData
        );
        ...
        console.log(
          "📦 [fetchStripeData] Subscriptions et invoices mises à jour dans le state"
        );
      } catch (error) {
        console.error(
          "❌ [fetchStripeData] Erreur lors de l'appel à Stripe :",
          error
        );
      }
    };
        ...
        <View style={{ padding: 16 }}>
          ...
          {stripeSubscriptions.length > 0 && (
            <View style={styles.card}>
              <Text style={styles.cardTitle}>Mes abonnements</Text>
              {stripeSubscriptions.map((sub) => {
                console.log("📦 Abonnement Stripe :", sub); // 👈 log ici
                ...
                return (
                  <View key={sub.id} style={styles.item}>
                    ...
                    {sub.items?.data?.map((item: any) => {
                      console.log("🧾 Item d'abonnement :", item); // 👈 log item
