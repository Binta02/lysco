...
export default function AdminDashboard() {
  ...
  onPress={async () => {
    if (!doc.file_url) {
      Alert.alert("Erreur", "Le lien du fichier est manquant.");
      return;
    }

    try {
      const result = await WebBrowser.openBrowserAsync(doc.file_url);
      console.log("✅ Fichier ouvert dans le navigateur :", result);
    } catch (err) {
      console.error("❌ Impossible d’ouvrir le fichier :", err);
      Alert.alert(
        "Erreur",
        "Impossible d'ouvrir le fichier. Copiez manuellement ce lien :\n" +
          doc.file_url
      );
    }
  }}
