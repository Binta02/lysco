...
export default function AdminDashboard() {
  ...
  const handleUpload = async () => {
    ...
    console.log("📥 Téléchargement du fichier en blob...");
    const response = await fetch(file.uri);
    const blob = await response.blob();

    const fileName = `${Date.now()}_${file.name}`;
    const path = `${selectedClient.id}/${fileName}`;
    console.log("🛣️ Chemin de destination :", path);
    ...
    console.log("🧾 Insertion dans la BDD user_documents...");
    ...
    if (insertError) {
      console.error("❌ Insertion BDD :", insertError);
    } else {
      console.log("✅ Insertion BDD réussie. Rafraîchissement de la liste...");
      await fetchDocuments();
    }
  };
