...
export default function AdminDashboard() {
  ...
  // };
  const handleDeleteDocument = async (docId: string, filePath: string) => {
    console.log("🗑️ Début suppression du document");
    console.log("📄 ID du document à supprimer :", docId);
    console.log("📂 Chemin du fichier dans le storage :", filePath);

    // 1. Supprimer du storage
    console.log("📦 Tentative de suppression dans Supabase Storage...");
    const { error: storageError } = await supabase.storage
      .from("documents")
      .remove([filePath]);
    ...
    if (storageError) {
      console.error(
        "❌ Erreur lors de la suppression du fichier dans le storage :",
        storageError
      );
    } else {
    ...
    // 2. Supprimer de la BDD
    console.log(
      "🗃️ Tentative de suppression dans la base de données Supabase..."
    );
    const { error: dbError } = await supabase
      .from("user_documents")
      .delete()
    ...
    if (dbError) {
      ...
      console.log(
        "✅ Enregistrement supprimé de la base de données avec succès !"
      );
    }

    // 3. Mise à jour des documents affichés
