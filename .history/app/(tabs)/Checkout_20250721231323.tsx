...
const Checkout = () => {
  ...
  const handleSubmit = async (data: FormValues) => {
    setIsProcessing(true);
    try {
      if (!stripe || !confirmPayment) throw new Error("Stripe non initialisé");

      // ✅ 0️⃣ Vérifier et récupérer userId Supabase
      const { data: sessionData } = await supabase.auth.getSession();
      const id = sessionData?.session?.user.id;
      if (!id) throw new Error("Utilisateur non connecté");

      // 1️⃣ Créer le PaymentMethod
      ...
      if (pmError || !paymentMethod) {
        throw new Error(pmError?.message || "Erreur création PaymentMethod");
      }
      ...
      // 3️⃣ Appel au backend pour récupérer clientSecrets
      const resp = await fetch(
        "https://mon-backend-node.vercel.app/api/create-payment-intent",
        {
          method: "POST",
          ...
          body: JSON.stringify({
            ...
            userId: id, // ✅ On utilise le vrai ID récupéré
            oneTimeItems,
            subscriptionItems,
            items,
            total,
      ...
      // 4️⃣ Confirmer les paiements one-time
      ...
      // 5️⃣ Confirmer l’abonnement s’il existe
      ...
      // 6️⃣ Enregistrer les commandes localement (Supabase)
      await Promise.all(
