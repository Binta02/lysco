...
const Checkout = () => {
  ...
  const handleSubmit = async (data: FormValues) => {
    ...
    try {
      ...
      // 1️⃣ Créer le PaymentMethod Stripe
      const { paymentMethod, error: pmError } = await confirmPayment(
        "pm_fake_client_secret_for_creation", // 🚧 à remplacer ou corriger côté backend
        {
          paymentMethodType: "Card",
          paymentMethodData: {
            billingDetails: {
      ...
      const subscriptionItems = items
        .map((item, index) => ({
          price: getPriceIdFromProductId(item.id) || "", // 👈 évite undefined
          quantity: item.quantity,
        }))
      ...
      console.log("✅ subscriptionPaymentIntent:", subscriptionPaymentIntent);
      ...
      for (const { clientSecret, index } of oneTimePaymentIntents) {
        ...
        if (confirmErr) {
          ...
          Alert.alert(
            "Erreur",
            confirmErr?.message || `Échec paiement one-time #${index}`
          );
      ...
      if (subscriptionPaymentIntent?.clientSecret) {
        ...
        if (subErr) {
          ...
          Alert.alert(
            "Erreur",
            subErr?.message || "Échec paiement abonnement."
          );
