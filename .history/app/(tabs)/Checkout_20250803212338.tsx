...
const Checkout = () => {
  ...
  const handleSubmit = async (data: FormValues) => {
    setIsProcessing(true);
    console.log("🚀 handleSubmit lancé avec :", data);

    const validationError = validateFormData(data);
    if (validationError) {
      console.warn("❌ Erreur validation :", validationError);
      Alert.alert("Erreur de validation", validationError);
      setIsProcessing(false);
      return;
    }
    ...
    try {
      ...
      if (!resp.ok) {
        const text = await resp.text();
        console.error("❌ Réponse backend non OK :", text);
        throw new Error("Backend error: " + text);
      }
      ...
      for (const { clientSecret, index } of oneTimePaymentIntents) {
        console.log(`💳 Confirmation du paiement item #${index}...`);
        const { error: confirmErr } = await confirmPayment(clientSecret, {
          paymentMethodType: "Card",
        });
      ...
      if (subscriptionPaymentIntent?.clientSecret) {
        console.log("💳 Confirmation du paiement abonnement...");
        const { error: subErr } = await confirmPayment(
          subscriptionPaymentIntent.clientSecret,
          {
            paymentMethodType: "Card",
        ...
        if (subErr) {
          console.error("❌ Erreur abonnement:", subErr.message);
          throw new Error(`Erreur abonnement: ${subErr.message || ""}`);
        }
      }

      console.log("🗃️ Paiement(s) terminé(s), insertion en base...");
      // Tu peux aussi ajouter des logs pour chaque type d’item ici si besoin
