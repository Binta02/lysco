...
const Checkout = () => {
  ...
  const handleSubmit = async (data: FormValues) => {
    console.log("🚀 handleSubmit démarré avec:", data);
    setIsProcessing(true);

    const validationError = validateFormData(data);
    if (validationError) {
      console.warn("⚠️ Erreur de validation:", validationError);
      Alert.alert("Erreur de validation", validationError);
      setIsProcessing(false);
      return;
    }
    ...
    if (!stripe || !confirmPayment) {
      console.warn("❌ Stripe ou confirmPayment non initialisé");
      Alert.alert("Erreur", "Stripe non initialisé.");
      setIsProcessing(false);
      return;
    }
    ...
    try {
      ...
      console.log("📡 Envoi vers backend avec:", {
        email: data.email,
        paymentMethodId: paymentMethod.id,
        userId,
        oneTimeItems,
      ...
      for (const { clientSecret, index } of oneTimePaymentIntents) {
        console.log(`💳 Paiement en cours pour l'item #${index}`);
        const { error: confirmErr } = await confirmPayment(clientSecret, {
          paymentMethodType: "Card",
        });
        if (confirmErr) {
        ...
        console.log(`✅ Paiement réussi pour l'item #${index}`);
      }

      if (subscriptionPaymentIntent?.clientSecret) {
        console.log("💳 Paiement en cours pour l'abonnement...");
        const { error: subErr } = await confirmPayment(
          subscriptionPaymentIntent.clientSecret,
          {
            paymentMethodType: "Card",
        ...
        console.log("✅ Abonnement payé avec succès");
      }

      console.log("🧾 Insertion des services/réservations/domiciliations...");
      await Promise.all(
        items.map(async (item, idx) => {
          const table = item.id.includes("domiciliation")
            ? "user_domiciliations"
          ...
          if (
            ...
            if (times.length === 1) {
              start = times[0];
              end = `${String(parseInt(times[0].split(":")[0]) + 1).padStart(
                2,
                "0"
              )}:00`;
              periodKey = "morning";
              ...
              start = times[0];
                ...
                parseInt(times[times.length - 1].split(":")[0]) + 1
              ).padStart(2, "0")}:00`;
              periodKey = times.length === 2 ? "morning" : "journée";
            } else if (item.title.includes("morning")) {
              start = "09:00";
            ...
            console.log("📦 Insertion réservation Supabase:", insertData);
            const { error } = await supabase
              .from("reservations")
              .insert(insertData);
            if (error) console.error("❌ Erreur réservation Supabase:", error);
            else console.log("✅ Réservation insérée !");
          } else if (item.id.includes("domiciliation")) {
            const insertData = {
              user_id: userId,
              address: data.address,
            ...
            console.log("📦 Insertion domiciliation Supabase:", insertData);
            const { error } = await supabase
              .from("user_domiciliations")
              .insert(insertData);
            if (error)
              console.error("❌ Erreur domiciliation Supabase:", error);
            else console.log("✅ Domiciliation insérée !");
          } else {
            const insertData = {
              user_id: userId,
              name: item.title,
            ...
            console.log("📦 Insertion service Supabase:", insertData);
            const { error } = await supabase
              .from("user_services")
              .insert(insertData);
            if (error) console.error("❌ Erreur service Supabase:", error);
            else console.log("✅ Service inséré !");
          }
        })
      );
      ...
      console.log("🧹 Nettoyage du panier, redirection vers Confirmation...");
      clearCart();
      Alert.alert("✅ Paiement réussi", "Merci pour votre commande !");
      router.push({
        pathname: "/(tabs)/Confirmation",
      ...
      console.error("❌ Erreur générale handleSubmit:", err);
      Alert.alert("Erreur", err.message || "Une erreur est survenue");
    } finally {
