...
const Dashboard = () => {
  ...
  const handleDeleteAccount = useCallback(() => {
    ...
    Toast.show({
      type: "info",
      text1: "Confirmation requise",
      text2: "Appuyez à nouveau pour confirmer.",
      onPress: () => {
        confirmDeleteAccount(safeProfile);
      },
      autoHide: false, // pour laisser l’utilisateur cliquer
    });
  }, [profile]);

  const confirmDeleteAccount = async (profile: Profile) => {
    try {
      const res = await fetch(
