...
const Dashboard = () => {
  ...
  console.log("🐾 [Dashboard] loading :", userDataLoading);
  console.log("🐾 [Dashboard] error :", userDataError);
  ...
  //     console.log("📦 Session récupérée (RN) :", session);
  //     setSession(session);
  //   });
  ...
  useEffect(() => {
    supabase.auth.getSession().then(async ({ data: { session } }) => {
      ...
      if (session) {
        // IMPORTANT : force le client à utiliser la session active
        ...
        if (error) {
          ...
          console.log("✅ Session active enregistrée pour les requêtes !");
        }
      }
    });
      ...
      if (session) {
        supabase.auth
          ...
          .then(({ error }) => {
            if (error) {
              console.error(
                "❌ Erreur lors du setSession (auth change) :",
                error
              );
            } else {
              console.log("✅ Session mise à jour après auth change !");
            }
          });
      }
    });
  ...
  const handleDeleteAccount = useCallback(() => {
    console.log("👉 [handleDeleteAccount] Bouton appuyé !");
    console.log("👉 [handleDeleteAccount] Profil actuel :", profile);
    ...
    console.log("✅ [handleDeleteAccount] Profil OK, ouverture du modal");
    setShowConfirmModal(true);
  }, [profile]);

  const confirmDeleteAccount = async (profile: any) => {
    console.log("🔥 confirmDeleteAccount appelé avec :", profile);
    try {
      const payload = {
        id: profile.id,
        email: profile.email,
      ...
      console.log("✅ Compte désactivé côté backend, déconnexion Supabase");
      await supabase.auth.signOut();

      console.log("🚀 Déconnexion Supabase réussie, toast + redirection");
      Toast.show({
        type: "success",
