...
const ServiceDetail: React.FC = () => {
  ...
  const calculPrix = () => {
    ...
    // Vérifie si l'id existe bien dans reservationPrices
    const priceConfig = reservationPrices[id as keyof ReservationPrices];

    if (!priceConfig) {
      return base; // Si pas de config spécifique, retourne prix de base
    }

    if (id === "coworking-space") {
      return (selectedHours.length || 1) * (priceConfig.hour || 0);
    }
    ...
    if (id === "formation-room") {
      ...
      if (modeReservation === "halfDay") {
        return priceConfig.halfDay || 0;
      }
      if (modeReservation === "fullDay") {
        return priceConfig.fullDay || 0;
      }
    }
              ...
              <div className="space-y-6">
                ...
                {[
                  ...
                  <div className="mt-8 space-y-4 p-5 border border-gray-200 rounded-lg">
                    ...
                    {reservationNotice &&
                      dateReservation &&
                      id !== "coworking-space" && ( // ⛔ ne pas afficher pour le coworking à l'heure
                        <div className="mt-4 text-red-600 font-medium text-center">
                          {reservationNotice}
                        </div>
                      )}
                ...
                <Button
                  onClick={() =>
                    shareContent({
                      ...
                      text: `Je vous recommande ce service: ${service.title}`,
                      url: `https://votre-app.com/service/${id}`,
                    })
                  }
                >
                  Partager ce service
