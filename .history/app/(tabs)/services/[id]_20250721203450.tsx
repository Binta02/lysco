...
export default function ServiceDetailPage() {
  ...
  const isReservable = ["coworking-space", "formation-room", "location-bureau"].includes(key);
const [reservationType, setReservationType] = React.useState<"hour" | "halfDay" | "fullDay">("hour");
const [quantity, setQuantity] = React.useState(1);


    const getReservationPrice = () => {
  const prices = reservationPrices[key];
  if (!prices) return parseFloat(service.price.replace(",", "."));
  return prices[reservationType] * quantity;
};
const handleAddToCart = () => {
  ...
  addItem({
    id: key,
    title: `${service.title} (${translateReservationType(reservationType)})`,
    price: totalPrice,
    quantity,
  });
  ...
  toast({
    ...
    description: `${service.title} - ${translateReservationType(reservationType)} ajouté au panier.`,
  });
};
  ...
  <View style={{ marginVertical: 12 }}>
    <Text style={{ marginBottom: 4, fontWeight: "600" }}>Type de réservation :</Text>
    <View style={{ flexDirection: "row", justifyContent: "space-around", marginBottom: 8 }}>
      {Object.keys(reservationPrices[key]).map((type) => (
        <TouchableOpacity
          key={type}
          style={{
            backgroundColor: reservationType === type ? "#5cb9bc" : "#e5e7eb",
            paddingVertical: 6,
            paddingHorizontal: 12,
            borderRadius: 20,
          }}
          onPress={() => setReservationType(type)}
        >
          <Text style={{ color: reservationType === type ? "#fff" : "#000" }}>
