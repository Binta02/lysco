...
import { ReservationPrices } from "@/src/types/database";
...
export default function ServiceDetailPage() {
  ...
  const [selectedHour, setSelectedHour] = React.useState<string>("09:00");
  const [reservationsByDay, setReservationsByDay] = useState<
    Record<string, { morning: boolean; afternoon: boolean }>
  >({});

  const isMorningReserved = (dayKey: any) =>
    reservationsByDay[dayKey]?.morning || false;

  const isAfternoonReserved = (dayKey: any) =>
    reservationsByDay[dayKey]?.afternoon || false;

  const isFullDayReserved = (dayKey: any) =>
    isMorningReserved(dayKey) && isAfternoonReserved(dayKey);
  ...
  const getReservationPrice = (): number => {
    if (
      isReservable &&
      key in reservationPrices &&
      reservationType in reservationPrices[key as keyof ReservationPrices]
    ) {
      const prices = reservationPrices[key as keyof ReservationPrices];
      const priceValue = prices[reservationType as keyof typeof prices];
      return priceValue ?? 0; // fallback si undefined
    }
  ...
  const fetchReservedPeriods = async () => {
    ...
    const reservationType = getReservationType(key);
  ...
  useEffect(() => {
    const loadReservations = async () => {
      const reservations = await fetchReservedPeriods();
      const processed = processReservations(reservations);
      setReservationsByDay(processed);
    };
    loadReservations();
  }, [key]);
                  ...
                  {isReservable && (
                    <View style={{ marginVertical: 12 }}>
                      ...
                      <View style={{ flexDirection: "row", flexWrap: "wrap" }}>
                        {Object.keys(
                          reservationPrices[key as keyof ReservationPrices]
                        ).map((type) => (
                          <TouchableOpacity
                            key={type}
                            style={{
                      ...
                      <Text style={{ textAlign: "center", marginTop: 12 }}>
                        Total : {getReservationPrice().toFixed(2)} €
                      </Text>
                    </View>
                  )}
                </View>
