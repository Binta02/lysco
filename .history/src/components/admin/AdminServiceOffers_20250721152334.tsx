...
const AdminServiceOffers = () => {
  ...
  useEffect(() => {
    ...
    // D’abord récupérer la session existante
    supabase.auth
      .getSession()
      .then(({ data: { session } }) => {
    ...
    // Écouter les changements SANS écraser si déjà défini
    const { data: subscription } = supabase.auth.onAuthStateChange(
      (event, newSession) => {
        console.log("📡 onAuthStateChange EVENT:", event);
        console.log("📡 onAuthStateChange SESSION:", newSession);
        setSession((prev) => prev || newSession); // <= IMPORTANT : ne pas écraser si déjà là
      }
    );
