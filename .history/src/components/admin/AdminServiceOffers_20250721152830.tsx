...
const AdminServiceOffers = () => {
  ...
  useEffect(() => {
    console.log("✅ DÉMARRAGE useEffect AdminServiceOffers");

    // 1️⃣ Écoute les changements de session
    const { data: subscription } = supabase.auth.onAuthStateChange(
      (_event, newSession) => {
        console.log("📡 onAuthStateChange EVENT:", _event);
        console.log("📡 onAuthStateChange SESSION:", newSession);
        setSession(newSession); // ✅ TOUJOURS prendre la dernière session reçue
      }
    );

    // 2️⃣ Récupère la session actuelle au démarrage
    supabase.auth
      .getSession()
      .then(({ data: { session } }) => {
    ...
    // 3️⃣ Nettoie l’abonnement
    return () => {
      console.log("🔌 UNSUBSCRIBE onAuthStateChange");
