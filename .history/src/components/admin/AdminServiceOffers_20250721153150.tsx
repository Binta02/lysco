...
const AdminServiceOffers = () => {
  ...
  useEffect(() => {
    console.log("✅ [AdminServiceOffers] DÉMARRAGE useEffect");

    // Check all AsyncStorage keys (optionnel mais utile)
    AsyncStorage.getAllKeys()
      .then(async (keys) => {
        const stores = await AsyncStorage.multiGet(keys);
        console.log("📦 [AdminServiceOffers] AsyncStorage contents:", stores);
      })
      .catch((err) => {
        console.error("❌ [AdminServiceOffers] AsyncStorage ERROR:", err);
      });

    // Get session at startup
    supabase.auth
      .getSession()
      .then(({ data: { session }, error }) => {
        console.log("🔑 [AdminServiceOffers] getSession RESULT:", session);
        if (error) {
          console.error(
            "❌ [AdminServiceOffers] getSession ERROR object:",
            error
          );
        }
        if (!session) {
          console.warn("⚠️ [AdminServiceOffers] No session found at startup.");
        } else {
          console.log(
            "✅ [AdminServiceOffers] Session user email:",
            session.user.email
          );
        }
        setSession(session);
      ...
      .catch((error) => {
        console.error("❌ [AdminServiceOffers] getSession CATCH ERROR:", error);
      });

    // Subscribe to auth state changes
    const { data: subscription } = supabase.auth.onAuthStateChange(
      (_event, newSession) => {
        console.log("📡 [AdminServiceOffers] onAuthStateChange EVENT:", _event);
        console.log(
          "📡 [AdminServiceOffers] onAuthStateChange SESSION:",
          newSession
        );
        if (newSession) {
          console.log(
            "✅ [AdminServiceOffers] New session user email:",
            newSession.user.email
          );
        } else {
          console.warn("⚠️ [AdminServiceOffers] Session became null.");
        }
        setSession(newSession);
      }
    );

    console.log("👀 [AdminServiceOffers] Subscription object:", subscription);

    return () => {
      console.log("🔌 [AdminServiceOffers] UNSUBSCRIBE onAuthStateChange");
