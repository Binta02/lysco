...
export default function ReservationForm({
  ...
  async function handleReservation() {
    ...
    if (selectedHours.includes("morning")) {
      start = `${selectedDate} 09:00:00+00`;
      end = `${selectedDate} 12:00:00+00`;
    } else if (selectedHours.includes("afternoon")) {
      start = `${selectedDate} 13:00:00+00`;
      end = `${selectedDate} 16:00:00+00`;
    } else if (
      selectedHours.includes("fullDay") ||
      ...
      start = `${selectedDate} ${startHour
        .toString()
        .padStart(2, "0")}:00:00+00`;
      end = `${selectedDate} ${endHour.toString().padStart(2, "0")}:00:00+00`;
    }
    ...
    const { error } = await supabase.from("reservations").insert({
      user_id: userId, // ✅ requis par la politique RLS
      reservation_date: selectedDate,
      reservation_type: serviceType,
      period: periodRange,
      price,
    ...
    <View style={{ padding: 20 }}>
      ...
      <Calendar
        ...
        onDayPress={(day) => {
          ...
          if (isSunday) {
            alert("❌ Les réservations sont indisponibles le dimanche.");
            return;
          }
          if (fullyBookedDates.includes(day.dateString)) return;
          setSelectedDate(day.dateString);
          ...
          return (
            <TouchableOpacity
              style={{
                ...
                borderRadius: 8,
                margin: 2,
                opacity: state === "disabled" || isSunday ? 0.3 : 1,
              }}
              disabled={
                state === "disabled" ||
                fullyBookedDates.includes(date.dateString) ||
                  ...
                  return (
                    <TouchableOpacity
                      ...
                      onPress={() => {
                        ...
                        if (opt === "fullDay") {
                          setSelectedHours(["fullDay"]); // nécessaire pour déclencher le bouton
                        } else {
                          setSelectedHours([]);
                        }
                      }}
