...
const ProductDescription = () => {
  ...
  const fetchReviews = async () => {
    try {
      setIsLoading(true);
      setReviews([]); // reset avant de charger

      if (!productId) {
        console.warn("productId manquant");
        return;
      }
      console.log("productId utilisé :", productId);

      const { data, error } = await supabase
        .from("reviews")
        .select("*")
        .filter("lower(product_id)", "eq", productId.toLowerCase()) // <- changement ici
        .order("created_at", { ascending: false });
