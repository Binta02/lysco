...
const ProductDescription = () => {
  ...
  const fetchReviews = async () => {
    try {
      ...
      console.log("🔎 Recherche des avis pour le produit :", productId);

      const { data, error } = await supabase
        .from("reviews")
        .select("*")
        .ilike("product_id", productId) // insensible à la casse
        .order("created_at", { ascending: false });
      ...
      console.log(`✅ ${data.length} avis récupérés depuis Supabase :`, data);
      ...
      const formattedReviews = await Promise.all(
        data.map(async (review) => {
          ...
          if (review.user_id) {
            console.log(
              `👤 Récupération du profil pour user_id : ${review.user_id}`
            );
      ...
      console.log("🎉 Avis formatés :", formattedReviews);
      setReviews(formattedReviews);
    } catch (error) {
      ...
      setIsLoading(false);
      console.log("✅ Fin du chargement des avis");
    }
  };
