...
export function useUserData(session: any) {
  ...
  async function ensureProfileExists(session: any) {
    console.log(
      "🔍 [ensureProfileExists] DÉBUT pour user_id :",
      session.user.id
    );

    try {
      console.log("➡️ Étape 1 : Vérifier si le profil existe déjà");
      ...
      console.log("✅ Résultat select profile :", profileData, profileError);

      if (profileError) {
        console.error(
          "❗ Erreur récup select profiles:",
          profileError,
          " (code:",
          profileError.code,
          ")"
      ...
      if (!profileData) {
        console.log("⚠ Aucun profil trouvé, on tente de créer une entrée");
        ...
        console.log("🛠️ Données à insérer :", newProfile);
        ...
        console.log("📥 Résultat insert :", insertData, insertError);
        ...
        console.log("✅ Profil déjà existant, pas besoin d'insérer.");
      }
    } catch (err) {
  ...
  const fetchUserData = async () => {
    ...
    try {
      ...
      console.log("✅ Session reçue dans hook:", session);
      ...
      if (!profileData) {
        ...
        return; // PAS de setProfile si vide !
      }
      setProfile({
        id: profileData.id,
