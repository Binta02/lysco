...
export function useUserData(session: any) {
  ...
  async function ensureProfileExists(session: any) {
    ...
    try {
      ...
      if (profileError) {
        console.log("‼️ profileError.code:", profileError.code);
      ...
      if (!profileData) {
        ...
        console.log("Supabase user id (auth.uid()):", session.user.id);
        console.log("🔍 session.user.id après INSERT:", session?.user?.id);
        ...
        if (insertError) {
          ...
          console.log("❗ insertError.hint:", insertError.hint);
          throw insertError;
        }
