...
export default function AdminDashboard() {
  ...
  //   console.log("🗑️ Début de la suppression du document...");
  //   console.log("📄 ID du document :", docId);
  //   console.log("📂 Chemin du fichier dans le storage :", filePath);

  //   // 🔍 Vérifie le contenu du dossier utilisateur
  //   const userFolder = filePath.split("/")[0];
  //   const listResult = await supabase.storage
  //     .from("documents")
  //     .list(userFolder);
  //   console.log("📂 Contenu actuel du dossier :", listResult);

  //   // 📦 Suppression du fichier dans le storage Supabase
  //   const { data: removedPaths, error: storageError } = await supabase.storage
  //     .from("documents")
  //     .remove([filePath]);

  //   if (storageError) {
  //     console.error("❌ Erreur Storage :", storageError);
  //   } else if (!removedPaths || removedPaths.length === 0) {
  //     console.warn("⚠️ Aucun fichier supprimé du storage !");
  //   } else {
  //     console.log("✅ Fichier supprimé du storage :", removedPaths);
  //   }

  //   // 🗃️ Suppression de la ligne dans la BDD (avec .select pour retour)
  //   const { data: deleteResult, error: dbError } = await supabase
  //     .from("user_documents")
  //     .delete()
  //     .eq("id", docId)
  //     .select(); // ⚠️ permet d'avoir deleteResult

  //   if (dbError) {
  //     console.error("❌ Erreur BDD :", dbError);
  //   } else if (!deleteResult || deleteResult.length === 0) {
  //     console.warn("⚠️ Aucune ligne supprimée dans la table user_documents !");
  //   } else {
  ...
  //     console.log("🔄 Rafraîchissement des documents en cours...");
  ...
  //     if (fetchError) {
  //       console.error("❌ Erreur pendant le rafraîchissement :", fetchError);
  //     } else {
  //       console.log("✅ Documents actualisés :", refreshedDocs);
  //       setDocuments(refreshedDocs || []);
  //     }
  //   }

  //   console.log("🎉 Fin de la procédure de suppression.");
  // };
